...
The following 21 additional transactions were signed locally and accepted by
the TBC testnet node. Together with the nine transactions above, this report
contains 30 accepted broadcasts.
...
All 21 accepted transactions satisfy the signed-size fee target. The MultiSig
...
The Rust OrderBook implementation represents the published TBC-to-FT order
family. Go retains the same protocol scope and intentionally excludes
