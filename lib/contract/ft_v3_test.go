...
func TestMintFTUsesReleasedV4Script(t *testing.T) {
	fx := newHTLCTokenFixture(t, 1_000)
	ft, err := NewFT(&FtParams{
		Name: "Released", Symbol: "FT4", Amount: 1_000, Decimal: 2,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if got := mint.Outputs[0].LockingScript.Len(); got != util.FTV4CodeLength {
		t.Fatalf("minted FT code length = %d, want %d", got, util.FTV4CodeLength)
	}
}
...
func TestComputeFtPartialHashUsesV4Boundary(t *testing.T) {
	...
	want := partialsha256.CalculatePartialHash(code.Bytes()[:util.FTV4PartialOffset])
	got, err := ComputeFtPartialHash(code.ToHex(), false)
	if err != nil {
		t.Fatal(err)
	}
	if got != want {
		t.Fatalf("FT v4 partial hash = %s, want %s", got, want)
	}
}
